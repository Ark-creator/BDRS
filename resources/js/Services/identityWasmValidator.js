...
import { getDocumentTypes, getDocumentTypeSync } from './documentTypeConfig';
import { isCapable as isWasmCapable } from './wasmDetect';
import { runPipeline } from './wasmPipeline';
...
const scoreDocumentType = async (rawText, profile) => {
    ...
    if (useGo) {
        try {
            const types = await getDocumentTypes();
            for (const [docType, p] of Object.entries(types)) {
    ...
    if (useGo) {
        try {
            ...
            if (goResult && goResult.confidence >= 24) {
                const profile = types[goResult.type];
                return {
                    type: goResult.type,
...
const extractFields = async (rawText, selectedType) => {
        ...
        try {
            ...
            if (goFields) {
                return {
                    ...
                    id_type: types[selectedType]?.label || null,
                };
            }
        } catch {
            // Fall through to JS
    ...
    const profile = types[selectedType];
...
const validateIdImage = async ({ role, file, validIdType, signal }) => {
    ...
    const expectedScore = await scoreDocumentType(ocrText, types[expectedType]);
...
export const processVerificationWithWasm = async (idFile, selfieFile, documentType) => {
    const wasmMode = window.__bdrsConfig?.wasmMode === true;
    const capable = await isWasmCapable();

    if (wasmMode && capable) {
        return runPipeline(idFile, selfieFile, documentType);
    }
