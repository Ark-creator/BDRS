...
const SendIcon = ({ processing }) => (
    <svg className={`w-6 h-6 transition-all duration-300 ease-in-out ${processing ? 'animate-spin' : ''}`} xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor">
...
export default function Messages() {
    ...
    const { data, setData, post, processing, reset } = useForm({ reply_message: '' });
    ...
    const handleReply = (e) => {
        ...
        post(route('admin.messages.storeReply', lastMessageInThread.id), {
            preserveScroll: true,
            onSuccess: () => {
                reset('reply_message');
                    ...
                    {activeConversation ? (
                        <div key={activeConversation.user.id} className="fixed inset-0 pt-16 md:relative md:pt-0 flex flex-col w-full h-full bg-slate-50 dark:bg-slate-900 animate-fade-in">
                            <div className="p-3 border-b border-slate-200 dark:border-slate-700 flex items-center gap-3 flex-shrink-0 bg-white dark:bg-slate-800 shadow-sm">
                                <button onClick={() => setActiveConversationId(null)} className="md:hidden p-1 rounded-full hover:bg-slate-200 dark:hover:bg-slate-700">
                                    <BackIcon className="w-6 h-6 text-slate-600 dark:text-slate-300" />
                                </button>
                                <UserAvatar user={activeConversation.user} />
                                <div>
                                    <h4 className="font-bold text-slate-900 dark:text-slate-100">{activeConversation.user.full_name}</h4>
                            ...
                            <div className="p-3 border-t border-slate-200 dark:border-slate-700 bg-white/80 dark:bg-slate-800/80 backdrop-blur-sm flex-shrink-0">
                                <form onSubmit={handleReply} className="flex items-end gap-3">
                                    <textarea ref={replyTextareaRef} value={data.reply_message} onInput={handleTextareaInput} className="w-full p-2.5 border-slate-300 dark:border-slate-600 bg-slate-100 dark:bg-slate-700 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-shadow resize-none max-h-40" rows="1" placeholder="Type your message..."/>
                                    <button type="submit" disabled={processing || !data.reply_message.trim()} className="p-2.5 bg-blue-600 text-white rounded-lg hover:bg-blue-700 disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-200 flex-shrink-0 shadow hover:shadow-md">
                                        <SendIcon processing={processing} />
                                    </button>
                                </form>
                            </div>
                        </div>
    ...
    return (
        <button onClick={() => onSelect(conv)} className={`w-full text-left p-3 border-b border-slate-200 dark:border-slate-700 transition-colors duration-150 relative flex items-center gap-3 ${isActive ? 'bg-blue-50 dark:bg-slate-900/50' : 'hover:bg-slate-100 dark:hover:bg-slate-700'}`}>
            ...
            <div className="relative flex-shrink-0">
                ...
                {unreadCount > 0 && <span className="absolute -top-0.5 -right-0.5 text-xxs bg-rose-500 text-white font-bold rounded-full w-4 h-4 flex items-center justify-center border-2 border-white dark:border-slate-800">{unreadCount}</span>}
            </div>
