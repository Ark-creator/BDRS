...
// --- BUONG CODE PARA SA STEP 1 (Hindi tinanggal) ---
...
// --- BUONG CODE PARA SA STEP 2 (Hindi tinanggal) ---
    ...
    <div className="space-y-4">
        ...
        <div>
            ...
            <div className="relative">
                <CustomTextInput id="phone_number" type="tel" icon={<PhoneIcon />} value={data.phone_number} onChange={(e) => setData('phone_number', e.target.value)} required error={errors.phone_number || (phoneValidation.status === 'invalid' && data.phone_number)} className="pr-12" />
                {!errors.phone_number && <ValidationIndicator status={phoneValidation.status} />}
            </div>
            {phoneValidation.status === 'invalid' && data.phone_number && <InputError message={phoneValidation.message} className="mt-2" />}
            <InputError message={errors.phone_number} className="mt-2" />
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
                <label htmlFor="birthday" className="font-medium text-slate-700 text-sm mb-2 block">Birthday</label>
                <CustomTextInput id="birthday" type="date" icon={<CalendarIcon />} value={data.birthday} onChange={(e) => setData('birthday', e.target.value)} required className="pr-2 text-sm h-12" error={errors.birthday || (data.birthday && !ageValidation.isValid)} />
            ...
            <div>
                ...
                <CustomSelect id="gender" icon={<GenderIcon />} value={data.gender} onChange={(e) => setData('gender', e.target.value)} required className="text-sm h-12" error={errors.gender}>
...
// --- BUONG CODE PARA SA STEP 3 (Hindi tinanggal) ---
const Step3_AccountCredentials = ({ data, setData, errors, passwordVisible, setPasswordVisible, confirmPasswordVisible, setConfirmPasswordVisible, passwordsDoNotMatch, emailValidation }) => (
    <div className="space-y-4">
        ...
        <div>
            ...
            <div className="relative">
                <CustomTextInput id="email" type="email" icon={<MailIcon />} value={data.email} onChange={(e) => setData('email', e.target.value)} required error={errors.email || (emailValidation.status === 'invalid' && data.email)} className="pr-12" />
                {!errors.email && <ValidationIndicator status={emailValidation.status} />}
            </div>
            {emailValidation.status === 'invalid' && data.email && <InputError message={emailValidation.message} className="mt-2" />}
...
export default function Register() {
    ...
    const stepTitles = { 1: "Step 1:", 2: "Step 2:", 3: "Step 3: " };

    useEffect(() => {
        if (!data.phone_number) { setPhoneValidation({ status: 'idle', message: '' }); return; }
        setPhoneValidation({ status: 'checking', message: '' });
        const timeoutId = setTimeout(() => {
            axios.post(route('validation.phone'), { phone_number: data.phone_number })
                 .then(response => { setPhoneValidation(response.data.is_taken ? { status: 'invalid', message: 'This phone number is already registered.' } : { status: 'valid', message: '' }); })
    ...
    useEffect(() => {
        ...
        if (!data.email || !isEmailValid) { setEmailValidation({ status: 'idle', message: '' }); return; }
        setEmailValidation({ status: 'checking', message: '' });
        const timeoutId = setTimeout(() => {
            axios.post(route('validation.email'), { email: data.email })
                 .then(response => { setEmailValidation(response.data.is_taken ? { status: 'invalid', message: 'This email is already registered.' } : { status: 'valid', message: '' }); })
    ...
    const renderStep = () => {
        switch (step) {
            case 1:
                return <Step1_BasicInfo data={data} setData={setData} errors={errors} />;
            case 2:
                return <Step2_PersonalDetails data={data} setData={setData} errors={errors} phoneValidation={phoneValidation} ageValidation={ageValidation} />;
            case 3:
                return <Step3_AccountCredentials data={data} setData={setData} errors={errors} passwordVisible={passwordVisible} setPasswordVisible={setPasswordVisible} confirmPasswordVisible={confirmPasswordVisible} setConfirmPasswordVisible={setConfirmPasswordVisible} passwordsDoNotMatch={passwordsDoNotMatch} emailValidation={emailValidation} />;
            default:
                return null;
        }
    };
                ...
                <div className="flex items-center justify-center min-h-screen p-4">
                    <div className="w-full max-w-4xl mx-auto bg-white rounded-2xl shadow-2xl overflow-hidden md:flex">
                        ...
                        <div className="w-full md:w-1/2 p-6 md:p-10 flex flex-col">
                            <div className="flex justify-center mb-6 md:hidden"><img src="/images/logo1.jpg" alt="Barangay Logo" className="h-16 w-16" /></div>
                                ...
                                <form onSubmit={submit}>
                                    ...
                                    {step === 3 && (
                                        <div className="mt-6 flex items-start">
                                            <div className="flex items-center h-5"><input id="terms" name="terms" type="checkbox" className="focus:ring-blue-500 h-4 w-4 text-blue-600 border-gray-300 rounded" checked={agreeToTerms} onChange={(e) => setAgreeToTerms(e.target.checked)} /></div>
                                            <div className="ml-3 text-sm"><label htmlFor="terms" className="text-slate-600"> I have read and agree to the{' '} <button type="button" onClick={() => setIsTermsModalOpen(true)} className="font-medium text-blue-600 hover:underline focus:outline-none" > Terms and Conditions </button>. </label></div>
                                        </div>
                                    )}
                                    <div className="pt-6 space-y-4">
                                        ...
                                        {step === 2 && (
                                            <div className="flex gap-4">
                                                ...
                                                <PrimaryButton type="button" onClick={nextStep} disabled={!isStep2Complete}>Next Step</PrimaryButton>
                                            </div>
                                        )}
                                        {step === 3 && (
                                             <div className="flex gap-4">
                                                ...
                                                <PrimaryButton
                                                    type="submit"
                                                    disabled={processing || !passwordValidation.isValid || passwordsDoNotMatch || !agreeToTerms || (phoneValidation.status !== 'valid' && data.phone_number !== '') || (emailValidation.status !== 'valid' && data.email !== '')}
                                                >
                                                    Register
                                                </PrimaryButton>
                                            </div>
