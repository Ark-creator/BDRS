...
func initWasmBindings() {
	...
	api.Set("collectBackIDEvidence", js.FuncOf(func(this js.Value, args []js.Value) interface{} {
		...
		barcodeLike := args[2].Truthy()
		expectedScore := args[3].Int()
		result := CollectBackIDEvidence(rawText, metrics, barcodeLike, expectedScore)
		data, _ := json.Marshal(result)
		return parseJSON(string(data))
	}))
	...
	js.Global().Get("console").Call("log", "BDRS WASM Validator v"+VersionString()+" loaded")
}
...
func extractRGBA(args []js.Value) ([]byte, int, int) {
	...
	bufLen := jsBuf.Get("byteLength").Int()
	if bufLen < byteLen {
		js.Global().Get("console").Call("warn",
			"BDRS WASM: RGBA buffer too small: got "+strconv.Itoa(bufLen)+", need "+strconv.Itoa(byteLen))
		return nil, 0, 0
	}
...
func qualityFromJS(v js.Value) QualityMetrics {
	...
	if err := json.Unmarshal([]byte(jsonStr), &m); err != nil {
		js.Global().Get("console").Call("warn", "BDRS WASM: qualityFromJS parse error: "+err.Error())
	}
	return m
}
